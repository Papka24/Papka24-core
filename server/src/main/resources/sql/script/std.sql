...
(
    ...
    WHERE (
      (
        (r.author = ? AND r.status < 20 ) AND (r.company_id = ? <#if isMy> OR r.company_id ISNULL </#if>) <#if tags> AND ((r.tags & ? ) > 0) </#if>
        <#if dateFrom> AND ( r.time >= ?) </#if> <#if dateTo> AND ( r.time <= ?) </#if>
      ) OR (
        (s.user_login = ?) AND (s.company_id = ? <#if isMy> OR s.company_id ISNULL </#if>) <#if tags> AND ((s.tags & ? ) > 0) </#if>
