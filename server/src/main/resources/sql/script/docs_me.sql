...
(
    ...
    WHERE (
        (s.user_login = ? AND s.status < 10) AND (s.company_id = ? <#if isMy> OR s.company_id ISNULL </#if> )
    ) <#if tags> AND ((s.tags & ? ) > 0)  </#if> <#if dateFrom> AND ( s.time >= ?) </#if> <#if dateTo> AND ( s.time <= ?) </#if>
